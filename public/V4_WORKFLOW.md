...
flowchart TD
    ...
    K --> K3["学校代表色 / 校徽色"]
    K --> K4["网页搜索学校建筑和关键符号"]
    K --> K5["下载官方/高可信/用户建筑参考图"]
    K --> K6["提取建筑名、形状、立面和细节"]

    K6 --> L{"是否有真实建筑参考？"}
    L -->|有| M["作为高保真校园建筑依据"]
    L -->|无| N["继续找图 / 换建筑 / 请用户补图"]
...
如果没有可靠视觉参考，不应直接把具体建筑弱化成小元素或抽象符号。V4.1 需要多做网页搜索、官方图集、校园地图、新闻图片或招生页面检索；仍然找不到时，换一个可验证的校园建筑，或者请用户提供真实建筑照片。学校建筑是生成依据，不是可有可无的装饰。
