...
5. After school is available, research the school. Load [references/research-rules.md](references/research-rules.md) when doing this.
   ...
   - For any building, crest, gate inscription, or other school-specific element that may become a visible focal point, use web search and download 1-3 official or high-confidence reference images before generation when the environment allows it.
   - Treat downloaded reference images as the source of truth for exact facade shape, crest structure, and readable text details.
   - The workflow may spend more time and one or more internal research passes here; high-fidelity school identity is more important than speed.
...
- V4.1 / photo-enhanced outputs must still visibly belong to the school. They should include restrained but clear school identity elements: school name, graduation year when applicable, motto or spirit keywords, official/school-color accents, and at least one verified school cue such as a referenced background landmark, small crest/seal, title-side emblem, footer line art, or architectural silhouette. Do not reduce V4.1 to a generic portrait with no school logo, motto, typography, or campus identity.
...
- If a building or crest is important enough that the user will likely inspect it closely, text-only research is not enough. Downloaded reference images are required before treating it as an exact visible element.
- If the workflow cannot download and cache trustworthy reference images for a school-specific element, downgrade that element from `exact visible landmark` to `background atmosphere` or omit it.
- If exact school landmarks are unavailable or too risky to render, fall back to the verified regional / city context of the school rather than inventing unrelated landmarks. The fallback must still feel local to the school's real setting.
...
- If the generation tool does not support direct reference-image conditioning, do not overclaim exactness: use the cached reference images only to extract stricter facade cues and then reduce the size, prominence, or readability of risky school elements.
- Place accuracy-critical crest or emblem elements in low-risk layout zones such as the right margin, right-lower corner, side badge, footer, or a clean title area when possible. Do not bury them inside the busiest double-exposure area or crowd them against the face.
